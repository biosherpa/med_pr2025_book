...
```{r}
df_iam <- read.csv ("df_iam.csv")desviacion_peso <- by(df_iam$peso, df_iam$sex, function(x) sd(x, na.rm = TRUE))
```
...
#Describe en dos grupos
stats_peso <- by(df_iam$peso, df_iam$sex, describe)

#QUINTILES CON BY

QUINTILES_peso <- by(df_iam$peso, df_iam$sex, function(x) quantile(x, probs = seq(0, 1, by = 0.20), na.rm = TRUE))


QUINTILES_peso2 <- by(df_iam$peso, df_iam$sex, function(x) describe(x, quant = c(0, 0.2, 0.4, 0.6, 0.8, 1), na.rm = TRUE))
